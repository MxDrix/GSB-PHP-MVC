...
*/	
	public function getLesInfosFicheFrais($idVisiteur,$mois){
		...
		from fichefrais inner join etat on fichefrais.idEtat = etat.id where fichefrais.idvisiteur ='$idVisiteur' and fichefrais.mois = '$mois'";
		$res = PdoGsb::$monPdo->query($req);
		$laLigne = $res->fetch();
		return $laLigne;
	}
   ...
   function supprimer($idVisteur,$mois){
    ...
    // fonction update fichefrais de VA a RB
        ...
        public function lstFraisRembourser(){
                ...
                $mois = $mois-1;
 		$req = "select id,mois,nom,prenom,nbJustificatifs,montantValide,dateModif,idEtat  from fichefrais, visiteur  where visiteur.id = fichefrais.idVisiteur and fichefrais.idEtat = 'RB'  order by dateModif desc,mois ";
                $res = PdoGsb::$monPdo->query($req);
		$lstFraisV = $res->fetchall();
		return $lstFraisV;           
        }
